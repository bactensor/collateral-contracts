...
def main():
    ...
    parser = argparse.ArgumentParser(description='Get collaterals for miners who deposited in a given block range')
    ...
    deposit_events = get_deposit_events(w3, args.contract_address, args.block_start, args.block_end)
    ...
    for miner_address in miner_addresses:
        ...
        results.append([
            miner_address,
            cumulative_deposits[miner_address],
            collateral
        ])
    
    # Write results to stdout
    writer = csv.writer(sys.stdout)
    writer.writerow(['miner_address', 'cumulative_amount_of_deposits', 'total_collateral_amount'])
