...
def deploy_on_devnet_and_get_bytecode(w3, contract_address):
    ...
    trustee, decision_timeout, min_collateral_increase = get_config(
        w3, contract_address
    )
    ...
    try:
        ...
        deploy_cmd = [
            ...
            '--constructor-args', f'{trustee}', f'{min_collateral_increase}', f'{decision_timeout}',
        ]
...
def verify_contract(contract_address, rpc_url):
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python verify_contract.py <contract_address> <rpc_url>")
        print("Example: python verify_contract.py 0x123...abc http://localhost:8545")
        sys.exit(1)
    ...
    verify_contract(contract_address, rpc_url)
